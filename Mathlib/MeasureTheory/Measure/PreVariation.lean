...
variable (f : preVariation' Z) (g : C(ℝ, ℝ))

instance instFunLike : FunLike (preVariation' Z) (Set Z) ℝ≥0∞ where
  coe := preVariation'.toFun
  coe_injective' f g h := by cases f; cases g; congr

@[simp]
lemma preVariation'_apply (s : Set Z) : f.toFun s = f s := by rfl
...
variable (f : preVariation' X)

/-- `preVariationAux` of the empty set is equal to zero. -/
lemma empty : f ∅ = 0 := by rw [← preVariation'_apply, f.eq_iSup_sum]; simp
